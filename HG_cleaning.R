...
PP <- PP %>%
  mutate(CF = ifelse(event_type %in% c("GOAL","SHOT","MISS","BLOCK") & event_team == PP_team, 1, 0))

#Group by power play line, summarize the TOI and corsi for, filter down to power play lines that played 
#at least two minutes, create a corsi for per 60 minutes variable, and sort
PP_group <- PP %>%
...
#Will do the same as above, but also create a running count variable and filter down to
#the top player on each team for each game
D_player_result <- D_player_join %>%
  filter(Position == "D") %>%
  arrange(game_id, team, desc(TOI)) %>%
